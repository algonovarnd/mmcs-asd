...
n + (n-1) + (n-2) + \dots + 1.
\]
Эта сумма представляет собой сумму первых \(n\) натуральных чисел и равна
\(
\frac{n(n+1)}{2}.
\)
...
\]
Положим \(n_0 = \max(n_1, n_2)\). Тогда для всех \(n \ge n_0\)
\[
c_1 g(n) \le f(n) \le c_2 g(n),
\]
...
При достаточно больших значениях n эта функция ограничена сверху и снизу величинами, пропорциональными \(n^2\). Следовательно,
\(T(n) = Θ(n^2)\).
...
Приведите пример двух положительных функций \(f\) и \(g\), для которых не выполняется ни  
\(f=O(g)\), ни \(g=O(f)\).
